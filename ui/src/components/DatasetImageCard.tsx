            ...
            <button
              ...
              onClick={() => {
                openConfirm({
                  ...
                  message: `确定要删除此${isItAVideo ? '视频' : '图片'}吗？此操作无法撤销。`,
                  type: 'warning',
                  confirmText: '删除',
                  onConfirm: () => {
                    apiClient
      ...
      >
        ...
        {(!inViewport || !isVisible) && isCaptionLoaded && (
          <div className="w-full h-full flex items-center justify-center text-gray-400">
            {isVisible ? '滚动到可见区域以编辑标注' : '显示内容以编辑标注'}
          </div>
        )}
        {!isCaptionLoaded && !caption && (
          <div className="w-full h-full flex items-center justify-center text-gray-400">正在加载标注...</div>
        )}
      </div>
    </div>
  );
