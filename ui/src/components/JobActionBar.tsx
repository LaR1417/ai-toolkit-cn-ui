...
export default function JobActionBar({
  ...
  return (
    <div className={`${className}`}>
      ...
      {canStop && (
        <Button
          onClick={() => {
            ...
            openConfirm({
              title: '停止任务',
              message: `确定要停止任务"${job.name}"吗？之后可以恢复。`,
              type: 'info',
              confirmText: '停止',
              onConfirm: async () => {
      ...
      <Button
        onClick={() => {
          let message = `确定要删除任务"${job.name}"吗？这也将从磁盘上永久删除。`;
          if (job.status === 'running') {
            message += ' 警告：任务正在运行。如果可以，请先停止任务。';
          }
          openConfirm({
            title: '删除任务',
            message: message,
        ...
        <MenuItems anchor="bottom" className="bg-gray-900 border border-gray-700 rounded shadow-lg w-48 px-2 py-2 mt-4">
          ...
          <MenuItem>
            <div
              ...
              onClick={() => {
                let message = `确定要将此任务标记为已停止吗？如果状态卡住，这将把任务状态设为"已停止"。只有在 100% 确定任务已停止时才这样做。这不会停止任务。`;
                openConfirm({
                  title: '标记任务为已停止',
                  message: message,
                  type: 'warning',
